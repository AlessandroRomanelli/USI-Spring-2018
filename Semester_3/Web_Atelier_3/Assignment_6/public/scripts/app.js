...
class App {
  ...
  save() {
    ...
    return function () {
      ...
      const div = document.getElementById('temporary-saves');

      const element = `
        <form class='submit-form' action='/favorites' method='POST' enctype='application/x-www-form-urlencoded'>
          <h3 style='text-align:center'>Snapshot</h3>
          <img src='${url}' style='display:block'></img>
          <fieldset>
            <label>Name: </label>
            <input type='text' name='title'></input>
          </fieldset>
          <input type='hidden' name='dataUrl' value='${url}'></input>
          <input type='submit' value='Send Favorite'></input>
        </form>
      `;
      ...
      container.classList.add('temporary-fav');
